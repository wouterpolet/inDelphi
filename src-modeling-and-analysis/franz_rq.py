...
def parse_data(deletions_df):     # input: MH-based deletions
...
#  microhomologies = deletions[deletions['homologyLength'] != 0]
  ...
  for id, exp in enumerate(exps):
    ...
    mh_exp_data = deletions_df[deletions_df['Sample_Name'] == exp][
...
M#Hless_deletions = MHless_deletions.drop(columns=['index', 'homologyLength', 'homologyGCContent'])
